...
export const updateVocabularyItem = async (
  ...
  try {
    ...
    if (data.lastReviewed) {
      updateDataForFirestore.lastReviewed = Timestamp.fromDate(
        data.lastReviewed
      );
    }
...
export const getVocabularyItems = async (userId: string) => {
  try {
    ...
    return querySnapshot.docs.map((doc) => {
      ...
      const itemWithDates = convertTimestampsToDates(data, [
        "createdAt",
        "lastReviewed",
      ]);
...
export const getFolderById = async (
  ...
  try {
    if (!userId || !folderId)
      throw new Error("userId y folderId son requeridos");
...
export const createStudySession = async (
  ...
  try {
    ...
    if (folderId) {
      filteredItems = vocabularyItems.filter(
    ...
    const sortedItems = filteredItems.sort((a, b) => {
      ...
      if (a.nextReviewDate) {
        if (a.nextReviewDate instanceof Date) {
          ...
          typeof a.nextReviewDate === "object" &&
          (a.nextReviewDate as Timestamp).toDate
        ) {
          aDate = (a.nextReviewDate as Timestamp).toDate();
        } else {
          console.warn(
            "nextReviewDate no es un objeto Date válido:",
            a.nextReviewDate
          );
        }
      }
      ...
      if (b.nextReviewDate) {
        if (b.nextReviewDate instanceof Date) {
          ...
          typeof b.nextReviewDate === "object" &&
          (b.nextReviewDate as Timestamp).toDate
        ) {
          bDate = (b.nextReviewDate as Timestamp).toDate();
        } else {
          console.warn(
            "nextReviewDate no es un objeto Date válido:",
            b.nextReviewDate
    ...
    await setDoc(
      sessionDoc,
      convertDatesToTimestamps(session as Record<string, unknown>, [
        "createdAt",
      ])
    );
...
export const getStudySession = async (
  ...
  try {
    ...
    return {
      ...
      createdAt:
        timestampToDate(sessionData.createdAt as Timestamp) || new Date(),
  ...
  try {
    ...
    await updateDoc(sessionDoc, {
      itemsReviewed: [
        ...
        convertDatesToTimestamps(result as unknown as Record<string, unknown>, [
          "answeredAt",
        ]),
      ],
...
export const updateUserProgress = async (
  ...
  try {
    ...
    const updateData: {
      [key: string]: number | Record<string, number> | Timestamp | undefined;
    } = {};
